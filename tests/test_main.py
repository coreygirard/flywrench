...
@given(lists(one_of(floats(), integers(), text(max_size=100)), max_size=10))
...
def test_hashing_unique(e):
    h1 = Cache().make_hash(e)
    h2 = Cache().make_hash(e + 1)
    assert h1 != h2


def test_basic():
    class Example(Flywrench):
        cache = Cache()
...
def test_custom_algorithm():
    class Example(Flywrench):
        cache = Cache(hash_algorithm=lambda x: 42)
